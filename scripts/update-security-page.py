...
CONTROLS = [
    ...
    ("Outside sharing, only with the person you name.",
     "When your team shares a document with someone outside your company, the link names that one person. They confirm a one-time code sent to their address before anything opens, the link expires on the date you choose, you can revoke it at any time, and PDFs are stamped with their address and the date. Your admins can turn outside sharing off entirely."),
    ("No standing access, even for us.",
     "LOVELEEDAY staff cannot see your account by default. Opening access takes a stated reason and a time limit of a few hours; your owners and admins are emailed the moment it opens, see everything done during it, and can end it early."),
    ("Stricter rules for regulated data.",
     "Accounts that hold regulated records, such as student, health or financial data, are classified as such: outside sharing starts off, any existing outside links are revoked, links last at most seven days, and staff access is capped at four hours."),
