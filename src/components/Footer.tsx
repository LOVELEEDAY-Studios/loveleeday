...
export function Footer() {
  return (
    <footer className="mt-auto bg-[var(--ink)] text-[var(--on-dark)]">
      <div className="shell pt-20 pb-10">
        <div className="grid gap-12 border-b border-[var(--ink-3)] pb-14 md:grid-cols-[1.4fr_repeat(3,1fr)]">
          <div>
            <Link href="/" className="flex items-center gap-2.5" aria-label="LOVELEEDAY Studios — home">
              <LogoMark size={24} />
              <span className="text-[13.5px] font-semibold">
                LOVELEEDAY<span className="text-[var(--on-dark-mu)]"> Studios</span>
              </span>
            </Link>
            <p className="mt-5 max-w-[30ch] text-[14px] leading-[1.55] text-[var(--on-dark-mu)]">
              Intelligence architecture and the production software that runs on top of it.
            </p>
            <p className="eyebrow mt-6 text-[var(--on-dark-dim)]">
              Kalamazoo, Michigan
            </p>
          </div>
          ...
          {COLUMNS.map((col) => (
            <div key={col.title}>
              <h2 className="eyebrow text-[var(--on-dark-dim)]">{col.title}</h2>
                ...
                {col.links.map((l) => (
                  <li key={l.label}>
                    {l.ext ? (
                      <a
                        href={l.href}
                        className="text-[14px] text-[var(--on-dark-mu)] transition-colors hover:text-[var(--on-dark)]"
        ...
        <div className="mt-8 flex flex-col gap-2 border-t border-[var(--ink-3)] pt-7 text-[12.5px] text-[var(--on-dark-dim)] sm:flex-row sm:items-center sm:justify-between">
